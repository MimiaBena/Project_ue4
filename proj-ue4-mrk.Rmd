---
title: "Entrepôt et fouille de données"
output: html_document
Auteurs: Benaouali Mokhtaria& belkacem Noor Elhouda
---
## 1-Introduction :
...
1.2- Analyse descriptive des données
	Pour la réalisation du projet, on a utilisé les données de la base Geofla:base de donné qui représente les déchets par établissement  et répartition du radon sur la départition .
...
La principale source de données de notre entrepôt de données  est la base Geofla et le site data,gouv.fr .Nous avons chargé une grande partie des informations  stockées dans cette base, notamment les données géographiques pour chaque département et le taux de déchets de chaque établissement .Dans notre cas on a utilisé la base des déchets radioactifs, la base de potentiel radon dans chaque commune et le registre des rejets et des transferts de polluants (RRTP).
...
```{r  image,echo=FALSE, fig.cap="A caption", out.width= 100%}
knitr::include_graphics("C:/Users/Mimia/Pictures/Documents/projet-ue4/model.png")
...
```
Par la suite ,on va sélectionner certaines colonnes nécéssaire pour les explorer et on supprime les doublants et les informations manquantes.

```{r }
radon_code <- radon_code %>% 
  select(insee_com,nom_comm, nom_dept, 
         classe_potentiel,coordonnees_gps1,coordonnees_gps2) %>%
  unique()

dechets_catalog <- dechets_catalog[!is.na(dechets_catalog$ACTIVITE...Bq.),]
```
Par la suite on a crée une fonction pour donner la couleur correspondant aux communes en fonction de la classe potentiel :
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
      ...
      plot(departements,col=getColor(catalogue_radon))
...
```
#Traitement du fichier des déchets radioactif:
# Representation des departements:
Dans cette partie on a travaillé sur le fichiers des déchets radioactifs en fonction des départements.
```{r }
...
#representation de la fonction getColor par rapport à l'activité des déchets radioactifs
getColor <- function(dechets_catalog_dep1) {
  sapply(dechets_catalog_dep1$ACTIVITE...Bq., function(ACTIVITE...Bq.) {
    if(ACTIVITE...Bq.<= 0) {
      "green"
    } else if(ACTIVITE...Bq.<=100000000) {
      "orange"
    } else {
      "red"
    } })
...
icons <- awesomeIcons(
  ...
  iconColor = 'black',
  library = 'ion',
  markerColor = getColor(dechets_catalog_dep1)
)
...
server <- function(input, output, session) {
  ...
  output$dechets_catalog <- renderLeaflet({
...
Dans cette partie ,lon a  representé de la'activité des dechets radioactifs en fonction des commune
```{r }
...
###deuxieme methode 
getColor <- function(dechets_catalog_comun) {
  sapply(dechets_catalog_comun$ACTIVITE...Bq., function(ACTIVITE...Bq.) {
    if(ACTIVITE...Bq.<= 1000) {
      "red"
    } else if(ACTIVITE...Bq.>1000) {
      "orange"
    } else {
      "green"
    } })
...
icons <- awesomeIcons(
  ...
  iconColor = 'black',
  library = 'ion',
  markerColor = getColor(dechets_catalog_comun)
)
...
##"eff_deprt <- t(eff_deprt)
###population dans chaque departement 
...
localisation <- dechets_catalog_dep1 %>% 
  select(CODE_DEPT,coordonnees_gps1,coordonnees_gps2)  %>%  unique()
...
stone.stat(calcul_popu, region=region, lambda=1)
...
calcul_popu1<-data.frame(Observed=calcul_popu$observed)
calcul_popu1<-cbind(calcul_popu1, Expected=calcul_popu$POPULATION*sum(calcul_popu$observed)/sum(calcul_popu$POPULATION))
                    
moranI.stat(data=calcul_popu1, listw=col.W, n=length(ncCR85.nb), S0=Szero(col.W) )
moranI.stat(data=calcul_popu1, applyto="residuals", listw=col.W, n=length(ncCR85.nb),
            S0=Szero(col.W) )
moranI.test(Observed~offset(log(Expected)), calcul_popu1, model="poisson", R=99,
